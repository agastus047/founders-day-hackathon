        ...
        {events.map((event, index) => (
          <div key={index} className="mb-4">
            <div className="flex items-center mb-2">
              <div className="bg-blue-700 text-white rounded-full h-8 w-8 flex items-center justify-center">
              </div>
              <div className="ml-4 text-xl text-white transition duration-300 ease-in transform hover:scale-105">{event.date}</div>
            </div>
            <div className="ml-12">
              <h2 className="text-xl md:text-2xl font-semibold text-gray-500 ">{event.title}</h2>
            </div>
          </div>
        ))}
      </div>
